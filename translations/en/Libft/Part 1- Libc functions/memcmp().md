`memcmp` compares the contents of the first `n` bytes of two blocks of memory, rather than comparing strings.
...
Its purpose is to compare the first `n` bytes of the two memory regions starting at `s1` and `s2`, meaning it compares at most that many bytes.
...
The C standard only guarantees the sign of the return value and 0, not the exact value returned. This means you should make one of the following checks instead of `memcmp(s1, s2, n) == -1`, because the C standard does not mandate returning -1:
...
| --------- | --------------- |
| `memset` | Sets a block of memory to a specific byte |
| `memcpy` | Copies a block of memory to another block of memory |
| `memmove` | Safely moves/copies memory that may overlap |
| `memchr` | Searches for a specific byte in memory |
| `memcmp` | Compares two blocks of memory |

The common feature of this family of functions is that they treat data as raw bytes rather than interpreting it as "strings".
...
| **Is `n` required?** | **Yes**, `n` specifies how many bytes to compare | **No** |
| **Is `\0` required?** | **No** | **Yes, strings must end with `\0`** |
| **When does it stop?** | After comparing `n` bytes, or upon encountering the first differing byte | Upon encountering the first differing character, or upon encountering `\0` |
...
| **Returns `0` when** | All first `n` bytes are identical | The contents of both strings are identical |
...
| **Typical Usage** | Comparing arrays, raw memory in structs, binary data, etc. | Comparing usernames, words, sentences, and other strings |
