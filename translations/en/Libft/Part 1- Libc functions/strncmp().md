...
Its function is to compare `s1` and `s2` byte by byte from the beginning, up to a maximum of `n` characters; it can stop upon encountering the first differing character or `\0`. If all characters within the comparison range are identical, it returns `0`; otherwise, it returns a negative or positive value based on the magnitude relationship of the first differing character.
...
`strcmp` compares entire strings, whereas `strncmp` compares at most the first n characters.
...
| Maximum characters to compare | Unlimited | n characters |
...
The 3 most critical points about `strncmp`:

- n is "the maximum number of characters to compare", not necessarily requiring n characters to be compared.
- Stops upon encountering the 1st differing character.
- The return value should only be checked for its sign; do not depend on specific numbers:
...
Because the standard does not guarantee it will strictly return -1.

The essence of `strncmp` is:

- Within a window of at most `n`, find the first differing character between `s1` and `s2`.
- If found, compare them; if not found, return 0.