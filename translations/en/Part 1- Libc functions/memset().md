...
```
Its function can be simply understood as setting ```c
#include <string.h>

void	*memset(void *s, int c, size_t n);
``` consecutive bytes starting from s to the lower 8 bits of c.
The lower 8 bits are the rightmost 8 bits in the binary representation of an integer. The reason memset takes these 8 bits is that it ultimately writes a byte (with a value range of 0 to 255), and 1 byte = 8 bits.
...
```txt
   ...
   起始地址    要设置的   设置多少
               字节值     个字节
```
Returns the starting address s of the modified memory area, whose type is `void *`, a generic memory address.
...
Therefore, 65 converted to hexadecimal is 0x41, and each byte will become 41 41 41 41 41, which is A A A A A
