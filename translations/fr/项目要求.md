Implémenter certaines fonctions en langage C, telles qu'une ré-implémentation de la libc, en créant un makefile, en exécutant la commande make, pour générer une bibliothèque statique publique `libft.a`.
<font color="red">Il s'agit de la bibliothèque de base de tout le Common Core.</font>
###### Groupe 1 : Caractères
```c
ft_isalpha
...
| `ft_lstiter`      | Parcourir la liste, exécuter une fonction sur chaque nœud |
...
2. **Les fonctions d'aide (Helper Functions) doivent être `static`**
Par exemple :
```c
ft_split()
```
...
Les objets de travail principaux de `ar` sont des `.o`
...
> `rcs` :
>	- `r` = *replace* : Ajoute les fichiers `.o` spécifiés à l'archive. Si un membre du même nom existe déjà dans l'archive, il est remplacé ; sinon, il est créé.
...
3. **Instructions d'utilisation**, telles que :
...
Lister les documents de référence utilisés pendant l'apprentissage, par exemple :
