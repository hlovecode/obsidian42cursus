...
```
Son rôle est très simple : il calcule le nombre de caractères dans une chaîne, à l'exclusion du `'\0` de fin. 
...
`strlen(NULL)` est illégal et constitue un comportement indéfini (*undefined behavior*), car `strlen` essaie d'accéder à un emplacement qui ne contient aucune chaîne valide. Par conséquent, n'écrivez pas :
```c
char *str = NULL;
strlen(str);
```
