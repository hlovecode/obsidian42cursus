`isascii()`, tout comme `isalpha()`, `isdigit()` et `isalnum()`, fait partie des **fonctions de classification de caractères (character classification functions)**, à une différence près très importante : `isascii()` ne teste pas si un caractère est une lettre ou un chiffre, mais si ce caractère est un caractère ASCII.
