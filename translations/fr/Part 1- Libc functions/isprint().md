`isprint()` et `isalpha()`, `isdigit()`, `isalnum()`, `isascii()` que vous avez étudiés précédemment appartiennent au même groupe de **fonctions de classification de caractères C (character classification functions)**, utilisées pour déterminer si un caractère est un **caractère imprimable (printing character)**.
