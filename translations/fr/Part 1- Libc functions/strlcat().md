`strlcat` : **Concaténer** une chaîne de caractères à la fin d'une chaîne cible
...
Il retourne la longueur totale de la chaîne qu'il a tenté de créer. Si dst et src sont des chaînes normales se terminant par '\0', il retourne `strlen(dst) + strlen(src)`. Attention, il s'agit de la longueur de dst avant la concaténation.

**Attention : le paramètre size est la capacité totale du 1er paramètre dst, et non la longueur à ajouter.**
...
char src[] = " World!!!"; // 长度是9
```
La longueur de chaîne retournée par strlcat(...) est 14, mais dst n'a en réalité pu devenir que "Hello Wo", d'une longueur de 8, et il retourne tout de même 14. Car la fonction vous indique : "Si l'espace avait été suffisant, j'aurais voulu obtenir une chaîne de longueur 14". On peut ainsi déterminer si une troncature de chaîne s'est produite :
```c
char dst[10] = "Hello";
char src[] = " World!!!";
...
7. 返回“原 dst 长度 + src 长度”
    ...
    dst_len          剩余空间
                  ...
                  size-dst_len-1
                    ...
                    追加 src
```
Dans `strlcat(dst, src, size)`, `size` est **la capacité totale du tampon dst**, tandis que la valeur de retour est **la longueur du `dst` d'origine + la longueur de `src`**, et non le nombre de caractères réellement ajoutés.