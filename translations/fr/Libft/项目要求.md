...
<font color="red">Il s'agit de la bibliothèque fondamentale de tout le Common Core.</font>
###### Groupe 1 : Caractères
=======
Implémenter certaines fonctions en langage C, telles que la réimplémentation de la libc, en créant un makefile, en exécutant la commande make, et en générant une bibliothèque statique publique libft.a.
<font color="red">C'est la bibliothèque de base de l'ensemble du cursus Common Core.</font>
###### Groupe 1 : Classification de caractères
>>>>>>> origin/main
```c
ft_isalpha
...
| `ft_lstiter`      | Parcourir la liste et exécuter une fonction pour chaque nœud       |
...
Puisque Libft est une bibliothèque publique, elle doit respecter le principe : même entrée -> toujours même sortie.
S'il y a des variables globales, leur modification entraînera une variation du résultat final.

2. **Les fonctions d'assistance (Helper Functions) doivent être déclarées `static`**
Par exemple : 
```c
ft_split()
```
...
| `ft_lstdelone`    | Supprimer un nœud          |
| `ft_lstclear`     | Supprimer toute la liste   |
| `ft_lstiter`      | Parcourir la liste et appliquer une fonction à chaque nœud |
| `ft_lstmap`       | Transformer le contenu de chaque nœud pour générer une nouvelle liste |
### 1. Considérations techniques

1. **Interdiction d'utiliser des variables globales**
Puisque Libft est une bibliothèque publique, une même entrée doit toujours produire la même sortie.
La présence de variables globales, en cas de modification de celles-ci, entraînerait une variation du résultat final.

2. **Les fonctions auxiliaires (Helper Functions) doivent être déclarées static**
Par exemple :
```c
ft_split()
```
...
```
<<<<<<< HEAD
Ces fonctions ne doivent pas être exposées aux autres, elles doivent être écrites ainsi :
=======
...
=======
Car le mot-clé static indique qu'elles <font color="red">ne peuvent être utilisées que dans ce fichier</font> et ne polluent pas l'ensemble de la bibliothèque.
...
```
<<<<<<< HEAD
Compiler sans avertissements (Warning).
...
Les objets de travail principaux de `ar` sont les `.o`
...
ar est l'abréviation d'archive. C'est un outil présent sous les systèmes Unix/Linux/macOS, utilisé pour regrouper plusieurs fichiers objets (`.o`) dans un fichier d'archive.
Les objets de travail principaux de ar sont les `.o`
...
<<<<<<< HEAD
`libft.a` n'est pas une bibliothèque en cours d'exécution, c'est en réalité un fichier d'archive contenant de nombreux fichiers `.o`, ce qui peut se comprendre grossière par :
=======
`libft.a` n'est pas une bibliothèque en cours d'exécution ; c'est en réalité un fichier d'archive contenant de nombreux fichiers `.o`. On peut schématiquement le comprendre ainsi :
>>>>>>> origin/main
```c
libft.a
│
...
> ```
<<<<<<< HEAD
> ar : Appeler l'outil d'archivage
> rcs :
...
```
<<<<<<< HEAD
Est la méthode la plus typique de création de bibliothèque statique, qui peut se comprendre comme :
=======
est la méthode la plus typique de création d'une bibliothèque statique, que l'on peut comprendre comme :
>>>>>>> origin/main
```bash
ar
│
...
Le projet exige explicitement l'utilisation de `ar` pour créer `libft.a`, l'utilisation de `libtool` est interdite.
...
| `cc` / `gcc` | Compiler le C |
| `ar`         | Créer / gérer l'archive      |
| linker (éditeur de liens) | Lier les fichiers objets/bibliothèques pour former le programme final |
Le projet exige explicitement l'utilisation de `ar` pour créer `libft.a`. L'utilisation de `libtool` est interdite.
...
```
En cas de travail en collaboration, plusieurs identifiants (*login*) peuvent être écrits à la suite.
...
De plus, il est obligatoire d'expliquer **l'utilisation de l'IA dans le projet**, par exemple pour l'explication de concepts, la relecture de code ou le débogage, tout en précisant quelles parties ont été réalisées par vous-même.
=======
1. **La première ligne doit être en italique et son contenu doit être strictement :**
```
*This activity has been created as part of the 42 curriculum by <login>.*
```
En cas de travail en équipe, plusieurs identifiants (login) peuvent être écrits à la suite.
