...
```
Son action peut être simplement comprise comme suit : à partir de s, définir les n octets consécutifs tous à la valeur des 8 bits de poids faible de c. 
Les 8 bits de poids faible correspondent aux 8 bits les plus à droite dans la représentation binaire d'un entier. Si memset utilise ces 8 bits, c'est parce qu'il doit finalement écrire un octet (dont la plage de valeurs va de 0 à 255), et que 1 octet = 8 bits. 
...
```txt
   ...
   起始地址    要设置的   设置多少
               字节值     个字节
```
Renvoie l'adresse de début s de la zone mémoire modifiée, de type `void *`, qui est une adresse mémoire générique. 

`memset` modifie n octets à partir d'une certaine adresse. Lorsque n == 0, cela signifie qu'on modifie 0 octet, rien n'est modifié, ce qui revient à ne rien faire. 
Attention, n est un nombre d'octets, et non un nombre d'éléments. 

`memset(s, c, n)` signifie : à partir de l'adresse `s`, écrire les `n` octets consécutifs avec les 8 bits de poids faible de `c`, et renvoyer `s`. Les « 8 bits de poids faible » sont les 8 bits les plus à droite de la représentation binaire d'un entier ; si `memset` utilise finalement cette partie, c'est qu'il convertit `c` en `unsigned char`, puis écrit de manière répétée dans la mémoire avec l'octet comme unité.

#### 2. Comprendre les paramètres de la fonction

1. Le 1er paramètre `void *s`
Représente l'adresse de début de la zone mémoire à manipuler. `void *` est utilisé car memset ne se soucie pas du type réel que vous passez ; il manipule des octets (bytes) et non des types C tels que char, int, double, etc.
Par exemple :
```c
char str[10];
int tab[10];
...
Définit les n octets à la valeur unsigned char obtenue après conversion de c. 
Par exemple :
```c
char str[5];
...
PS : Généralement, un int fait 4 octets.
...
4. Configurer un bloc de mémoire avec un octet spécifique
```c
char buffer[10];
...
└──────┴──────┴──────┴──────┘
```
C'est cela `memset(str, 0x12345678, 4);`.

PS : Comprendre l'écriture 0x78
0x -> Indique au compilateur que le nombre 78 qui suit est exprimé en hexadécimal. L'utilisation de 0x est une convention du langage C.
