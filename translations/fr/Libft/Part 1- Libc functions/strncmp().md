...
Sa fonction est de comparer octet par octet et depuis le début `s1` et `s2`, au maximum `n` caractères ; elle s'arrête au premier caractère différent ou à `\0`. Si tous les caractères comparés sont identiques, elle retourne `0`, sinon elle retourne une valeur négative ou positive selon la relation de grandeur du premier caractère différent.
...
Remarque : ne dépendez pas d'une valeur exacte de -1 ou 1, la norme garantit uniquement < 0, = 0 ou > 0.
