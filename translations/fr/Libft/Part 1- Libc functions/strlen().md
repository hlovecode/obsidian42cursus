...
Les chaînes de caractères en langage C ne constituent pas un type de données indépendant, ce sont en réalité des suites de `char`. 
...
Le paramètre de la fonction doit pointer vers une chaîne C valide et terminée par `\0`.
...
`strlen(NULL)` est illégal et constitue un comportement indéfini (*undefined behavior*), car `strlen` essaiera d'accéder à une adresse qui ne correspond à aucune chaîne valide. Par conséquent, il ne faut pas écrire :
...
Il s'agit d'un type d'entier non signé `unsigned int` ou `unsigned long`, déclaré et défini par `<stddef.h>`. C'est le type le plus sûr pour représenter tout objet de données entier utilisé comme indice de tableau, sans avoir à craindre qu'un petit tableau ne devienne très grand au fil des modifications du programme.

Lors de l'utilisation de `size_t`, l'arithmétique des indices ne subira jamais de dépassement (*overflow*). Partout dans le programme où l'on effectue des opérations arithmétiques sur des indices de tableaux ou des adresses, on devrait utiliser le type `size_t`. Son seul inconvénient est qu'il ne peut pas accepter de valeurs négatives. 
