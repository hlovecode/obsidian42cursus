strdup() a un rôle très direct : il duplique une chaîne de caractères et alloue dynamiquement de la mémoire pour la chaîne dupliquée.
...
**strdup retourne finalement l'adresse de début de la chaîne nouvellement dupliquée et ne modifie pas la chaîne d'origine**
...
`strdup` : Alloue sa propre nouvelle mémoire, puis y copie src.

|               | strcpy   | strdup  |
| ------------- | -------- | ------- |
| Copie la chaîne       | Yes      | Yes     |
| Alloue de la nouvelle mémoire       | No       | Yes     |
| dest doit-il exister au préalable | Yes      | No      |
| Valeur de retour           | char \*  | char \* |
| Doit-on faire un free après utilisation  | Dépend de dest | Oui      |