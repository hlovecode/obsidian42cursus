Implémenter certaines fonctions en langage C, telles que la réimplémentation de la libc, en créant un makefile, en exécutant la commande make, pour générer une bibliothèque statique publique libft.a.
<font color="red">Ceci est la bibliothèque de base de l'ensemble du Common Core.</font>
###### Groupe 1 : Caractères
```c
ft_isalpha
...
| `ft_lstclear`     | Supprimer toute la liste chaînée               |
| `ft_lstiter`      | Parcourir la liste chaînée et exécuter une fonction sur chaque nœud       |
| `ft_lstmap`       | Transformer le contenu de chaque nœud pour générer une nouvelle liste chaînée |
### 1. Considérations techniques (Technical considerations)

1. **Interdiction d'utiliser des variables globales**
Puisque Libft est une bibliothèque publique, elle doit toujours produire la même sortie pour la même entrée.
...
static free_all()
```
Parce que static signifie <font color="red">utilisable uniquement dans ce fichier</font>, évitant de polluer l'ensemble de la bibliothèque.
...
...
```
Tout placer directement dans le répertoire racine.

4. **Interdiction de soumettre des fichiers inutiles**
Par exemple :
```c
test.c
old.c
...
-Werror
```
Pouvoir être compilés sans avertissement (Warning).

6. **Utilisation obligatoire de la commande ar**
ar est l'abréviation de archive. C'est un outil sous les systèmes Unix/Linux/macOS utilisé pour empaqueter plusieurs fichiers objets (`.o`) dans un fichier archive.
...
`libft.a` n'est pas une bibliothèque en cours d'exécution, c'est en fait un fichier archive contenant de nombreux fichiers `.o`, ce qui peut se comprendre grossièrement par :
```c
libft.a
│
├── ft_strlen.o
...
>	ar rcs libft.a *.o
> ```
> ar : Appeler l'outil d'archivage (archive)
> rcs :
...
ar rcs libft.a *.o
```
est la méthode de création de bibliothèque statique la plus typique, qui peut être comprise comme :
```bash
ar
│
├── r → 把 .o 加进去 / 替换旧版本
...
Le projet exige explicitement l'utilisation de `ar` pour créer `libft.a`, et interdit l'utilisation de `libtool`
...
1. **La 1ère ligne doit être en italique, et le contenu doit obligatoirement être :**
```
*This activity has been created as part of the 42 curriculum by <login>.*
```
En cas de travail en équipe, plusieurs logins peuvent être écrits à la suite.
...
- Quelles catégories de fonctions sont incluses
