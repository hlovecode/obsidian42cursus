...
Étant donné que Libft est une bibliothèque publique, elle doit respecter le principe : même entrée -> toujours même sortie.
...
static free_all()
```
Car le mot-clé `static` signifie <font color="red">utilisable uniquement dans ce fichier</font> et ne pollue pas l'ensemble de la bibliothèque.
...
5. **Tous les fichiers `.c` doivent obligatoirement :**
```bash
-Wall
-Wextra
-Werror
```
Pouvoir être compilés sans avertissements (no warning).
...
`libft.a` n'est pas une bibliothèque en cours d'exécution ; il s'agit en réalité d'un fichier d'archive contenant de nombreux fichiers `.o`, ce qui peut se comprendre grossièrement par :
```c
libft.a
│
├── ft_strlen.o
...
> `rcs` :
>	- `r` = replace (remplacer) : ajoute les fichiers `.o` spécifiés à l'archive. Si un membre portant le même nom existe déjà dans l'archive, il est remplacé ; sinon, il est créé.
...
Lister les ressources consultées pendant l'apprentissage, par exemple :
- La documentation de la bibliothèque standard C (pages man)
- Des tutoriels
- Des articles techniques, etc.
En outre, il est obligatoire d'expliquer **l'utilisation de l'IA dans le projet**, par exemple si elle a été utilisée pour l'explication de concepts, la revue de code ou le débogage, tout en précisant quelles parties ont été réalisées par vous-même.
